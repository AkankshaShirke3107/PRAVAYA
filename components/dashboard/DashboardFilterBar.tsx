...
export function DashboardFilterBar() {
  ...
  const [customEndDate, setCustomEndDate] = useState<string>('2026-05-28');
  ...
  const handleApply = () => {
    ...
    if (site !== 'All Sites') count++;
    if (selectedReportTypes.length < 3) count++;
    if (riskLevel !== 'All Levels') count++;
  ...
  return (
    <div className="rounded-xl border bg-card/95 p-3.5 shadow-sm backdrop-blur-sm space-y-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Select value={dateRange} onValueChange={setDateRange}>
              <SelectTrigger className="h-9 min-w-[130px] sm:min-w-[145px] text-xs font-medium bg-background">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Select value={site} onValueChange={setSite}>
              <SelectTrigger className="h-9 min-w-[125px] sm:min-w-[140px] text-xs font-medium bg-background">
          ...
          <div className="flex items-center gap-2.5 rounded-md border border-input bg-background px-3 py-1.5 text-xs shadow-xs">
            ...
            <div className="flex items-center gap-3">
              {reportTypeOptions.map((opt) => {
                ...
                return (
                  <label
                    key={opt.value}
                    className="flex items-center gap-1.5 cursor-pointer select-none text-xs font-medium text-foreground hover:text-sky-600 dark:hover:text-sky-400 transition-colors"
                  >
                    <input
                      type="checkbox"
                      checked={isChecked}
                      onChange={() => handleToggleReportType(opt.value)}
                      className="h-3.5 w-3.5 rounded border-muted-foreground/40 text-sky-600 focus:ring-sky-500 cursor-pointer accent-sky-600"
                    />
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Select value={riskLevel} onValueChange={setRiskLevel}>
              <SelectTrigger className="h-9 min-w-[120px] sm:min-w-[135px] text-xs font-medium bg-background">
      ...
      {/* Expandable Custom Date Range Sub-row (if 'Custom range' is selected) */}
      {dateRange === 'custom' && (
        <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-border/50 text-xs text-muted-foreground animate-in fade-in-50 duration-200">
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-1 text-[11px]">
              ...
              <input
                ...
                className="h-7 rounded-md border border-input bg-background px-2 text-xs font-mono text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
              />
            </label>
            <label className="flex items-center gap-1 text-[11px]">
              To:
