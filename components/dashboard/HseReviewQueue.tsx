...
export function HseReviewQueue() {
  ...
  const getPriorityBadgeClass = (priority: 'P1' | 'P2' | 'P3') => {
    switch (priority) {
      case 'P1':
        return 'bg-[#C92925] text-white border-[#C92925] font-bold rounded-[2px]';
      case 'P2':
        return 'bg-[#D97706] text-white border-[#D97706] font-bold rounded-[2px]';
      case 'P3':
        return 'bg-[#2E7D32] text-white border-[#2E7D32] font-bold rounded-[2px]';
      default:
        return 'bg-muted text-muted-foreground rounded-[2px]';
    }
  };
    ...
    <Card className="flex flex-col panel-card panel-accent-red">
        ...
        <div className="flex flex-col w-full relative">
          ...
          <CardTitle className="text-sm font-semibold text-foreground flex justify-between items-center w-full">
            ...
            <Link
              ...
              className="text-xs font-semibold text-muted-foreground hover:text-[#C92925] inline-flex items-center gap-1 hover:underline self-start sm:self-auto"
            >
      ...
      <CardContent className="p-0 min-w-0">
        <div className="overflow-x-auto min-w-0">
          <Table>
            <TableHeader className="bg-[#082735] dark:bg-[#0B1520]">
              <TableRow className="text-xs border-b border-border hover:bg-transparent">
                <TableHead className="w-[100px] font-bold text-white hover:text-white">Priority</TableHead>
                <TableHead className="w-[130px] font-bold text-white hover:text-white">Report ID</TableHead>
                <TableHead className="min-w-[150px] font-bold text-white hover:text-white">Site</TableHead>
                <TableHead className="min-w-[160px] font-bold text-white hover:text-white">Precursor</TableHead>
                <TableHead className="w-[110px] font-bold text-white hover:text-white">Status</TableHead>
                <TableHead className="w-[80px] text-right pr-4 font-bold text-white hover:text-white">Inspect</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody className="text-xs">
              {sampleReviewQueue.map((item, idx) => (
                ...
                >
                  ...
                  {/* Report ID */}
                  <TableCell className="font-mono text-xs font-bold text-[#102F3E] dark:text-[#60C0D8] group-hover:underline group-hover:text-[#C92925] whitespace-nowrap py-3">
                    {item.id}
                  </TableCell>
        ...
        <div className="p-3 border-t border-border bg-muted/40 flex items-center justify-between">
          ...
          <Link
            href="/reports?status=Pending"
            className="text-xs font-semibold text-foreground hover:text-[#C92925] hover:underline flex items-center gap-1"
          >
            View All Reports
            <ChevronRight className="h-3.5 w-3.5" />
          </Link>
